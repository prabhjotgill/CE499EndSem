...
%% simulate axial force and moments without confining effect
r=80;         % number of rotations
forces=length(Pu); % nummber of axial load cases, only 1 in our case

%declaring empty variables in advanced for speedy exicution of loops
M=zeros(forces,r); % matrix for storing moment values
M1=0; P1=0;        % moment and load variables of each strain profiles, we will keep refreshing it to zero after profile change 
Phi=zeros(1,r);    %  matrix for storing phi values
P=zeros(forces,r); % matrix for storing load values, no actual purpose to store these, but for observation purpose while coding 
for z=1:forces
 ep=0;  % strain at mid section
 n=0; c=0; d=0; a=1; % iteration variables
...
while a<=r;
    ...
    for j=1:length(dd) 
        y=dd(j);
                ...
                e=ep-(y-(.5)*D)*a*.000002;     % strain at depth y, at rotation a
    ...
    [fcc, fuc ,fy] = fcstress1(e);    % confined, unconfined and steel stress
      ...
      % finding M and Phi
        ...
        pstrip=fy*ss(j) +fuc*Cn(j); % force generated by 1mm strip
        
        P1=P1+pstrip;               % accumulating force of strips to get force of section 
    
    M1=M1+ pstrip*(D/2-y);          % accumulating moment of strips to get moment of section
    end
    if abs(P1-Pu(z))<1000  % if inside 1 Kn (tolerance limit) of P, we are correct, we can record M and fi values
    M(z,a)=M1;
    P(z,a)=P1;
    ...
    elseif P1< Pu(z)  % we failed to get acceptable P, now change the central strain to attain desired P value
         ep=ep+.0001/(1+.2*(c*d));
         M1=0; P1=0;  % reseting the  M and P values for next profile
    ...
    else      % we failed to get acceptable P, now change the central strain to attain desired P value
        ep=ep - .0001/(1+.2*(c*d)); % decreasing central strain 
        M1=0; P1=0;
...
figure;
plot([0,Phi(1,:)],[0,M(z,:)])
ylabel('MOMENT "N-mm"'); xlabel('PHI "per-mm"'); % plot of single condition  
end
...
figure;
plot([0,Phi(1,:)],[0,MC(z,:)])
ylabel('CONFINED MOMENT "N-mm"'); xlabel('PHI "per-mm"');   
end
%M=[zeros(3,1),M];
Phi=[zeros(1,1),Phi];
figure; % combined plot
plot(Phi(1:80),M(1,:), Phi(1:80),MC(1,:)); ylabel('MOMENT "N-mm"'); xlabel('PHI "per-mm"');  
legend('UNCONFINED MOMENT','CONFINED MOMENT')  
